...
CREATE USER mmuser WITH PASSWORD 'a_strong_password_grin';
...
wget -O /tmp/mattermost-$MATTERMOST_VERSION-linux-amd64.tar.gz https://releases.mattermost.com/$MATTERMOST_VERSION/mattermost-$MATTERMOST_VERSION-linux-amd64.tar.gz
cd /tmp && tar xzf mattermost-$MATTERMOST_VERSION-linux-amd64.tar.gz
...
sudo sed -i -e 's/\/mattermost_test\?/\/mattermost/g' /opt/mattermost/config/config.json

cat <<'EOF' | sudo tee /opt/mattermost/config/config.json
{
  "ServiceSettings": {
  ...
  "SqlSettings": {
    "DriverName": "postgres",
    "DataSource": "postgres://mmuser:a_strong_password_grin@localhost/mattermost?sslmode=disable\u0026connect_timeout=10\u0026binary_parameters=yes",
    "DataSourceReplicas": [],
